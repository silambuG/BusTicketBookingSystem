using System;
using System.Collections.Generic;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Metadata;

namespace BusTicketBookingSystem.Models
{
    public partial class BusTicketBookingContext : DbContext
    {
        public BusTicketBookingContext()
        {
        }

        public BusTicketBookingContext(DbContextOptions<BusTicketBookingContext> options)
            : base(options)
        {
        }

        public virtual DbSet<SeatDetail> SeatDetails { get; set; } = null!;
        public virtual DbSet<UserDetail> UserDetails { get; set; } = null!;

        protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
        {
            if (!optionsBuilder.IsConfigured)
            {
#warning To protect potentially sensitive information in your connection string, you should move it out of source code. You can avoid scaffolding the connection string by using the Name= syntax to read it from configuration - see https://go.microsoft.com/fwlink/?linkid=2131148. For more guidance on storing connection strings, see http://go.microsoft.com/fwlink/?LinkId=723263.
                optionsBuilder.UseSqlServer("Server=SYNCLAPN-29936\\SQL2019;Database=BusTicketBooking;Trusted_Connection=True;TrustServerCertificate=True;");
            }
        }

        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            modelBuilder.Entity<SeatDetail>(entity =>
            {
                entity.HasKey(e => e.SeatId)
                    .HasName("PK__SeatDeta__311713D32687E805");

                entity.Property(e => e.SeatId)
                    .ValueGeneratedNever()
                    .HasColumnName("SeatID");

                entity.Property(e => e.SeatType)
                    .HasMaxLength(50)
                    .IsUnicode(false);
            });

            modelBuilder.Entity<UserDetail>(entity =>
            {
                entity.HasKey(e => e.UserId);

                entity.Property(e => e.UserId)
                    .ValueGeneratedNever()
                    .HasColumnName("UserID");

                entity.Property(e => e.CurentAddress)
                    .HasMaxLength(100)
                    .IsUnicode(false);

                entity.Property(e => e.From)
                    .HasMaxLength(50)
                    .IsUnicode(false)
                    .HasDefaultValueSql("('Chennai')");

                entity.Property(e => e.NativeAddress)
                    .HasMaxLength(100)
                    .IsUnicode(false);

                entity.Property(e => e.To)
                    .HasMaxLength(50)
                    .IsUnicode(false);

                entity.Property(e => e.UmobileNumber).HasColumnName("UMobileNumber");

                entity.Property(e => e.Uname)
                    .HasMaxLength(50)
                    .IsUnicode(false)
                    .HasColumnName("UName");

                entity.HasOne(d => d.SeatNumberNavigation)
                    .WithMany(p => p.InverseSeatNumberNavigation)
                    .HasForeignKey(d => d.SeatNumber)
                    .OnDelete(DeleteBehavior.ClientSetNull)
                    .HasConstraintName("FK__UserDetai__SeatN__276EDEB3");
            });

            OnModelCreatingPartial(modelBuilder);
        }

        partial void OnModelCreatingPartial(ModelBuilder modelBuilder);
    }
}
